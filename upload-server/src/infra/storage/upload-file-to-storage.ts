...
export async function uploadFileToStorage(input: UploadFileToStorageInput) {
  ...
  const sanitizeFileNameWithExtention = sanitizedFileName.concat(fileExtension)

  const uniqueFileName = `${folder}/${randomUUID()}-${sanitizeFileNameWithExtention}`
  ...
  return {
    key: uniqueFileName,
    url: new URL(uniqueFileName, env.CLOUDFLARE_PUBLIC_URL).toJSON().toString(),
  }
}
